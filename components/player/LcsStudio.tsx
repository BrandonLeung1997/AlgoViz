...
const DEFAULT_X = "ABCD";
const DEFAULT_Y = "ACBD";

export function LcsStudio() {
  ...
  const generateSteps = useCallback(() => generateLcsSteps(x, y), [x, y]);
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-4 py-6 lg:grid-cols-[1.4fr_1fr]">
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-slate-600">{lcsMeta.summary}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <TableCanvas step={playback.step} />
        </div>
        <ControlPanel
