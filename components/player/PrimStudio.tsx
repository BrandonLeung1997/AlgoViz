...
export function PrimStudio() {
  const [graph, setGraph] = useState(DEFAULT_DIJKSTRA_GRAPH);
  const [graphText, setGraphText] = useState(
    formatGraphInput(DEFAULT_DIJKSTRA_GRAPH),
  );
  ...
  const generateSteps = useCallback(
    () => generatePrimSteps(graph, source),
    [graph, source],
  );
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-4 py-6 lg:grid-cols-[1.4fr_1fr]">
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-slate-600">{primMeta.summary}</p>
        </div>
        ...
        <p className="-mt-2 text-xs text-slate-500">
          ...
          Max 12 nodes. Non-negative integer weights only. Edge lists are
          treated as undirected. Prim grows from the source; Kruskal sorts
          globally instead.
        </p>
        <ArrayInput
          label="Source"
